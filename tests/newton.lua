...
--------------------------------------------------------------------------------
-- Example 1: sqrt(5) (vector with single element)
--------------------------------------------------------------------------------
...
-- Run root finder
local res = newton(f5,df5,xini,opts)

printf(
  "sqrt(5):    iter = %3u\tx = %.3f\t\terror = %.3e\tdx = %.3e\n",
...
opts = {maxiter = 1000, epsabs = 1e-7, stop_fn = roots.test_residual}
...
-- Run root finder
res = newton(fR,dfR,xini,opts)

local x = res.x
printf(
  "rosenbrock: iter = %3u\tx = (%.3f,%.3f)\tf(x) = (%.3e,%.3e)\t  dx = %.3e\n",
  res.iter,x[1],x[2],res.f(x)[1],res.f(x)[2],res.dx:norm())
