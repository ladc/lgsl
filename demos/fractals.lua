...
local pi,sqrt,sin,cos,atan2 = math.pi, math.sqrt,math.sin, math.cos, math.atan2
local ilist = iter.ilist
local plot, ipath, path, rgb, rgba, rect, color_function =
  graph.plot, graph.ipath, graph.path, graph.rgb, graph.rgba, graph.rect, graph.color_function

local function c_generator(n, n_angle, len_frac, g)
   local exp, real, imag = complex.exp, complex.real, complex.imag
   local w, r, k = ilist(function() return 0 end, n+1), #g

   local s = len_frac^n
   local sz = matrix.cnew(n_angle, 1, function(k) return s * exp(2i*pi*(k-1)/n_angle) end)

   local sh = ilist(function(k) return g[(k-1)%r+1] - g[(k-2)%r+1] end, r)
...
local function pitag_tree_symm_demo()
   ...
   cf = function(d) return cfgen(1-d/depth) end
   local pl = plot()
   pl.units = false
