...
local bmat = matrix.cnew(4, 1, function(i) return i == 4 and 1 or 0 end)
...
--   local ell, erl = v1, roots[n-1]
   local ell, erl = 25, 35
   local n = #roots
   local lno = graph.fxline(function(x) return edet(x)/edet_scale(x) end, v2, erl)
   local lnd = graph.fxline(function(x) return edet_sub(x)/edet_sub_scale(x) end, ell, v2)
...
local function wave_demo()
   ...
   pcs:addline(graph.fxline(function(x) return (csqrn(fcs(x))) end, x1, x2))
