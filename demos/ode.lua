...
local function poincareplot(f, t0, t1, x0, y0, h0, tsmp, T)
   local s = ode {N= 2, eps_abs= 1e-8, method='rk8pd'}
...
local function f_vanderpol_gen(mu)
...
local function demo2()
   ...
   local s = ode {N= 2, eps_abs= 1e-8, methdo='rk8pd'}
   s:init(t0, h, f, x0, y0)
...
local function demo6()
   ...
   local o = ode {N= 3, eps_abs= 1e-8, method='rk8pd'}
   local step = o.step
   local lnxy = graph.path(x0, y0)
   local lnxz = graph.path(x0, z0)
   local lnyz = graph.path(y0, z0)

   o:init(t0, h0, f, x0, y0, z0)

   for t, y1, y2, y3 in o:evolve(t1, 1e-3) do
      lnxy:line_to(y1, y2)
      lnxz:line_to(y1, y3)
      lnyz:line_to(y2, y3)
   end
